...
Any failure of the prose provider — Ollama not running, a model that ignored
the marker protocol so badly the caller's parse raised — degrades to the
previous provider rather than to a 500. When both lanes are the same provider
(PROSE_PROVIDER unset) there is nothing to fall back to, so the error
...
def prose_completion(system: str, messages: list[dict], max_tokens: int = 1024) -> str:
