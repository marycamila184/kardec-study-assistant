...
- `mode_detector.py` — `detect_suggested_mode(question)`: regex-based intent detection returning `"estudar_obra"` (item-lookup intent, e.g. "questão 132", "item 45"), `"refletir"` (situational/emotional cues, e.g. "tenho medo", "perdi minha mãe"), or `None`. `estudar_obra` wins when both match (item-lookup is the more specific intent). Situational patterns are intentionally broad — a false positive only surfaces an optional client button, never changes an answer. No LLM cost. `extract_study_reference(question)`: returns `{"item_number", "book"}` extracted by regex (mapped to the canonical `BOOK_NAME_MAP` names) so the client can open `/study` directly from the suggestion without re-parsing the question. The book is the one explicitly named; when none is named, the "questão N" / "Q. N" forms default to O Livro dos Espíritos (universal spiritist convention — LE is the only work whose entries are called questões, numbered globally 1-1019), while "item N" keeps `book: null`. All patterns are accent-tolerant ("questao" matches) and must stay in sync between detection and extraction — a mismatch makes the /chat direct lookup fire without the client button, or vice versa.
- `query_condenser.py` — `condense_query(question, history)`: rewrites a follow-up + conversation history into a standalone search query (Groq, `condenser_model`) before embedding. The prompt pins the output to Portuguese and forbids replacing doctrine terms with generic synonyms. Shared by `/chat` (`generator.py`) and multi-turn `/reflect` (`reflect.py`); callers log (`logger.exception`) and fall back to the raw text if the condense call fails.
...
- `explicador_prompt.py` — Socratic tutor system prompt. Output JSON: `{"contexto", "conceitos_chave": [...]}` — `perguntas` was removed from the prompt (the frontend never displayed it; saves tokens/latency), but `parse_explicador_json` still tolerates and returns it (`[]` when absent) and the API field remains for compatibility. `contexto` (3-6 sentences) explains where the item fits in the doctrine and may include well-known historical/cultural background using general knowledge (e.g. who the Pharisees were) — but doctrine itself stays strictly grounded in the retrieved text, and the response must make that distinction legible in prose ("Historicamente, os fariseus eram... O texto, por sua vez, mostra que..."). `conceitos_chave` definitions may include a short clarifying gloss, not just a literal one-sentence extraction. It is still forbidden to summarize or paraphrase the passage itself, and to personify "o Espiritismo" as an agent that does/values things — attribute doctrinal claims to the passage, the text, or Kardec. `parse_explicador_json` returns `(contexto, conceitos_chave, perguntas)`.
...
- `reflect_prompt.py` — tone-adaptive system prompt with a hard no-advice constraint. `CLINICAL_KEYWORDS` (`vozes`, `sombras`, `pânico`, etc.) triggers an optional medical/mediumship caveat. `CRISIS_KEYWORDS` (suicidal-ideation/self-harm cues, with unaccented variants) drives `needs_crisis_note()`; matching appends the fixed `CRISIS_NOTE` (CVV 188 / SAMU 192) deterministically in the pipelines — never left to the LLM. `build_reflect_messages(..., force_closing=True)` injects an "ENCERRAMENTO OBRIGATÓRIO" directive so the model writes an actual farewell message when the round cap is hit. Output JSON: `{"opening", "doctrine_connection", "reflection_questions": [...], "is_closing"}` — 1 to 3 questions on every turn (fewer, sharper questions preferred; "uma conversa acolhedora não é um questionário"). `parse_reflect_json` extracts the four fields.
- `reflect.py` — pipeline: query condensation for multi-turn threads (`condense_query`, only when `conversation_history` is non-empty; first-turn reflections retrieve on the raw situation) → semantic retrieval (top 5) → primary chunks ([:2]) used for LLM → complementary chunks ([2:5]) passed to `curar()`. After `CAP_ROUNDS` (5) completed rounds, `force_closing` is passed into the prompt (so the closing is written as one) and also enforced post-hoc as a backstop. Crisis detection runs on situation + full history; `CRISIS_NOTE` is appended to `doctrine_connection` on every return path, including `not_found` and failures. The `not_found` message keeps the mode's warm tone (thanks the person for sharing, invites rephrasing, points to Abrir o Evangelho). Returns `opening`, `doctrine_connection`, `reflection_questions`, `is_closing`, `complementary_items`, `sources`, `not_found`, `generation_failed`.

**Generator** (`/chat`):
- `prompt.py` — system prompt + message builder for `/chat`. Rules: answer strictly from retrieved passages; separate original text from AI explanation; never close with unsolicited advice or a suggested course of action (only if the user's question asks for one directly); never personify "o Espiritismo" as an agent; never end the answer text with a question to the user (follow-up prompts live exclusively in the `[SEGUIR]` trailer, rendered as tappable chips); must end with two machine-readable trailer lines: `[FONTES: 1, 3]` listing the passage numbers actually used (`[FONTES:]` empty when none) and `[SEGUIR: pergunta 1 | pergunta 2]` with two short follow-up questions answerable from the works (preferably tied to the retrieved passages, never repeating or rephrasing a question already asked/answered in the conversation).
- `generator.py` — optional query condensation (`condense_query`, when history is present) → retrieval → prompt → Groq LLM answer. When the question references a specific item and the book is known (named in the question via `extract_study_reference`, or implied by `book_filter`), the item's chunks are fetched directly with `retrieve_by_item` and placed at the front of the passage list (semantic duplicates dropped) — semantic search can't reliably find an item by number, especially after query condensation. The direct lookup also rescues the answer when semantic retrieval returns nothing or fails. Returns `answer`, `sources` (each with `book`, `chapter`, `item_number`, `excerpt` — the retrieved chunk text, used by the frontend's citation-chip modal), `not_found`. Same crisis handling as `/reflect`: `needs_crisis_note(question)` appends `CRISIS_NOTE` to the answer on every return path (including `not_found`), and a crisis match also forces the medical caveat instruction into the prompt. `_strip_trailing_markers` strips the `[FONTES: ...]` and `[SEGUIR: ...]` trailers (either order) off the answer: FONTES filters `sources` to the passages the model says it used (fallbacks: no marker or only-invalid indices → all chunks; explicitly empty marker → no sources); SEGUIR yields `suggested_questions` (max 2; missing/malformed marker → `[]`). Suggestions are suppressed on crisis, `not_found`, and generation failure. The `not_found` answer is warm and points to reformulating or the Estudar uma Obra mode.
...
**Reflect mode guardrails:** The system prompt contains a verbatim Portuguese prohibition against advice, suggestions, medication recommendations, or any course of action. A keyword list triggers an optional one-sentence medical/mediumship caveat appended to the response. Suicidal-ideation/self-harm cues additionally append the fixed `CRISIS_NOTE` (CVV 188) deterministically in code, in both `/reflect` and `/chat`, on every return path. Same "never personify Espiritismo" rule as `/chat` and Explicador applies here too.
...
|---|---|---|
| `POST` | `/chat` | Tirar uma Dúvida — returns `answer`, `sources` (with `excerpt`), `suggested_questions` (up to 2 tappable follow-up chips, latest answer only in the client), `not_found`, `suggested_mode`, `suggested_item_number`, `suggested_book` |
