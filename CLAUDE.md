...
- **Crisis handling is deterministic, never left to the LLM:** `needs_crisis_note()` (suicidal-ideation/self-harm cues, accent-tolerant) appends the fixed `CRISIS_NOTE` (CVV 188 / SAMU 192) in code on **every** return path of both `/chat` and `/reflect`, and forces the medical caveat into the prompt. Suggestions/nudges are suppressed on crisis turns.
