...
| [2026-08-27-lembrete-push-design](2026-08-27-lembrete-push-design.md) | O lembrete volta por Web Push, e com ele **o primeiro armazenamento da história do projeto**: `{endpoint, chaves, hora, fuso, last_seen}` por aparelho, num store que não cruza com nada — nunca com `session_id`, nunca com o log de turnos. Apaga ao desligar, no 410 e aos 90 dias sem uso. O service worker registra só `push` e `notificationclick`, sem `fetch` handler, que é o que reconcilia isto com a decisão de não ter SW em [2026-08-27-pwa](2026-08-27-pwa-instalavel-design.md). No iPhone o botão só aparece com o app instalado | vale |
| [2026-08-29-lembrete-de-manha-e-reflexao-em-cache-design](2026-08-29-lembrete-de-manha-e-reflexao-em-cache-design.md) | Refina a spec acima ao produto real — **um lembrete por dia, de manhã, no Brasil**. Agendador de hora em hora com janela de 60 e seletor de horas cheias: isso **aposenta os dois defeitos aceitos** (o atraso de ~14 min e o disparo duplo do horário de verão) sem precisar do sexto campo, porque nenhum dos dois precisava dele — precisavam de um produto mais estreito. Todo fuso do Brasil e da lusofonia é hora cheia, medido. A notificação passa a levar o **título do capítulo** (o texto do trecho não cabe, e cortá-lo repetiria o erro de [2026-08-05](2026-08-05-curadoria-trecho-e-trilhas-design.md)). E a primeira explicação do dia vai para cache no Firestore, chaveada pela identidade da passagem — medido que `streamStudy` não leva perfil nem histórico, logo é idêntica para todo leitor | **pendente** |
