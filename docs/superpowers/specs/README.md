...
| [2026-08-27-pwa-instalavel-design](2026-08-27-pwa-instalavel-design.md) | Ícone na tela de início: manifest, ícones e favicon (que não existia). **Sem service worker** — medido, os assets `_astro` já são `immutable` por um ano, então não há o que acelerar, e a falha característica dele é fixar versão velha em silêncio. O lembrete fica de fora de propósito: é [2026-08-05](2026-08-05-desligar-lembrete-design.md), decidida por privacidade, e a ordem não custa nada porque no iOS o push só existe para app já instalado | **pendente** |
